...
public class CalcGrammar extends Grammar<CalcItem, CalcAttribute> {
	public CalcGrammar() {
		super(
			Set.of(CalcItem.TERM_0, CalcItem.TERM_1, CalcItem.TERM_2, 
			CalcItem.TERM_3, CalcItem.TERM_5, CalcItem.TERM_6, CalcItem.TERM_4, 
			CalcItem.TERM_7),
	...
	protected void initProducts() {
		...
		create(new Product<>(CalcItem.term, CalcItem.term, CalcItem.TERM_2, CalcItem.factor), p -> {
			p.left.val = p.right.get(0).val * p.right.get(2).val;
		});
		...
		create(new Product<>(CalcItem.factor, CalcItem.TERM_4), p -> {
